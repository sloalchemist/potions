...
describe('Item Position Gathering Tests', () => {
  describe('Test Finding Open Position', () => {
    test('Desired placement position is open', () => {
      ...
      const openPosition: Coord | undefined = Item.findEmptyPosition(desiredPosition);
      expect(openPosition).not.toBeUndefined();
      expect(JSON.stringify(desiredPosition)).toBe(JSON.stringify(openPosition));
    });
    ...
    test('Desired placement position is taken', () => {
        ...
        const openPosition: Coord | undefined = Item.findEmptyPosition(desiredPosition);
        expect(openPosition).not.toBeUndefined();
        expect(JSON.stringify(desiredPosition)).not.toBe(JSON.stringify(openPosition));
    });
