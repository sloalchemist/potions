...
describe('FrenchRomanNames', () => {
  ...
  it('should generate a valid name with random prefix and suffix', () => {
    ...
    const validPrefix = [
      'Aurelius', 'Claudius', 'Flavius', 'Julius', 'Lucius', 'Marcus', 'Octavius', 'Quintus', 'Tiberius', 'Valerius',
      'Beau', 'Chateau', 'Fleur', 'Jardin', 'Lumiere', 'Mont', 'Riviere', 'Val', 'Vigne'
    ];

    // Valid suffixes (French & Roman)
    const validSuffix = [
      'us', 'ium', 'ianus', 'ensis', 'or', 'ar', 'ax', 'ius', 'ae', 'a',
      'mont', 'ville', 'fort', 'chateau', 'lac', 'terre', 'bois', 'plaine', 'roc', 'vallee'
    ];
  ...
  it('should generate names with a mix of Roman and French prefixes and suffixes', () => {
    ...
    const isRomanPrefix = name.startsWith('Aurelius') || name.startsWith('Claudius') || name.startsWith('Flavius') ||
      name.startsWith('Julius') || name.startsWith('Lucius') || name.startsWith('Marcus') || name.startsWith('Octavius') ||
      name.startsWith('Quintus') || name.startsWith('Tiberius') || name.startsWith('Valerius');
    const isFrenchPrefix = name.startsWith('Beau') || name.startsWith('Chateau') || name.startsWith('Fleur') ||
      name.startsWith('Jardin') || name.startsWith('Lumiere') || name.startsWith('Mont') || name.startsWith('Riviere') ||
      name.startsWith('Val') || name.startsWith('Vigne');
      
    const isRomanSuffix = name.endsWith('us') || name.endsWith('ium') || name.endsWith('ianus') || 
      name.endsWith('ensis') || name.endsWith('or') || name.endsWith('ar') || name.endsWith('ax') || 
      name.endsWith('ius') || name.endsWith('ae') || name.endsWith('a');
    
    const isFrenchSuffix = name.endsWith('mont') || name.endsWith('ville') || name.endsWith('fort') || 
      name.endsWith('chateau') || name.endsWith('lac') || name.endsWith('terre') || name.endsWith('bois') ||
      name.endsWith('plaine') || name.endsWith('roc') || name.endsWith('vallee');
  ...
  it('should generate a name with the correct number of syllables', () => {
    ...
    const name = frenchRomanNames.generateRandomSyllableName(minSyllables, maxSyllables);
